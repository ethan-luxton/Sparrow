...
interface CalendarArgs {
  action: 'list_calendars' | 'list_events' | 'create_event' | 'delete_event' | 'update_event' | 'quick_add';
  calendarId?: string;
  eventId?: string;
  maxResults?: number;
  timeMin?: string;
...
export function calendarTool(): ToolDefinition {
  return {
    ...
    handler: async (args: CalendarArgs) => {
      ...
      const buildEventTime = (value?: string) => {
        if (!value) return null;
        if (dateOnlyRe.test(value)) return { date: value };
        if (dateTimeRe.test(value)) return { dateTime: value, timeZone: tz };
        return null;
      };

      const drive = google.drive({ version: 'v3', auth });
      const assertWithinBase = (target: string) => {
      ...
      const resolveAttachments = async () => {
        ...
        const result: Array<{ fileUrl: string; title?: string; mimeType?: string }> = [];
        for (const att of args.attachments) {
          ...
          if (att.fileId) {
            ...
            if (!fileUrl) throw new Error('Drive file does not expose webViewLink for attachment.');
            result.push({
              fileUrl,
          ...
          if (att.localPath) {
            ...
            if (!fileUrl) throw new Error('Drive upload did not return webViewLink for attachment.');
            result.push({
              fileUrl,
              title: name,
      ...
      switch (args.action) {
        ...
        case 'create_event': {
          if (args.confirm !== true) return 'Set confirm=true to create an event.';
          const calId = args.calendarId ?? 'primary';
        ...
        case 'delete_event': {
          if (args.confirm !== true) return 'Set confirm=true to delete an event.';
          const calId = args.calendarId ?? 'primary';
          if (!args.eventId) return 'eventId required';
          await calendar.events.delete({ calendarId: calId, eventId: args.eventId });
          return { deleted: true, id: args.eventId };
        }
        case 'update_event': {
          if (args.confirm !== true) return 'Set confirm=true to update an event.';
          const calId = args.calendarId ?? 'primary';
          if (!args.eventId) return 'eventId required';
          ...
          if (args.end) {
            const end = buildEventTime(args.end);
            if (!end) return 'end must be RFC3339 date-time or YYYY-MM-DD (all-day).';
            requestBody.end = end;
          }
          const attachments = await resolveAttachments();
          if (attachments?.length) requestBody.attachments = attachments;
          if (Object.keys(requestBody).length === 0) return 'No fields provided to update.';
        ...
        case 'quick_add': {
          if (args.confirm !== true) return 'Set confirm=true to quick add an event.';
          const calId = args.calendarId ?? 'primary';
          if (!args.text) return 'text required for quick_add.';

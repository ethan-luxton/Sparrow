...
export function filesystemTool(): ToolDefinition {
  return {
    name: 'filesystem',
    description: 'Safe local file access restricted to ~/.pixeltrail only. read/list/read_pdf_text/read_docx_text are read-only; write/write_pdf/write_binary require confirm=true.',
    permission: 'write',
    schema: {
      type: 'object',
      properties: {

...
export function googleDriveTool(): ToolDefinition {
  return {
    name: 'google_drive',
    description:
      'Interact with Google Drive: list, search, metadata, export docs, extract text, upload, create docs/folders. Write actions (upload/create/delete/download) require confirm=true.',
    permission: 'write',
    schema: {
      type: 'object',
      properties: {
    ...
    handler: async (args) => {
      ...
      switch (args.action) {
        ...
        case 'metadata': {
          if (!args.fileId) throw new Error('fileId required');
          const res = await drive.files.get({ fileId: args.fileId, fields: 'id, name, mimeType, size, owners, createdTime, modifiedTime' });
          return res.data;
        }
        case 'export_doc': {
          if (!args.fileId) throw new Error('fileId required');
          const res = await drive.files.export({ fileId: args.fileId, mimeType: 'text/plain' }, { responseType: 'text' as any });
          return res.data as unknown as string;
        }
        case 'export_pdf': {
          if (!args.fileId) throw new Error('fileId required');
          const res = await drive.files.export({ fileId: args.fileId, mimeType: 'application/pdf' }, { responseType: 'arraybuffer' as any });
          return Buffer.from(res.data as any).toString('base64');
        }
        case 'export_docx': {
          if (!args.fileId) throw new Error('fileId required');
          const res = await drive.files.export(
            { fileId: args.fileId, mimeType: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' },
        ...
        case 'upload': {
          if (!args.localPath) throw new Error('localPath required');
        ...
        case 'extract_text': {
          ...
          if (mimeType === 'application/pdf') {
            const res = await drive.files.get({ fileId: args.fileId, alt: 'media' }, { responseType: 'arraybuffer' as any });
            const buffer = Buffer.from(res.data as any);
        ...
        case 'delete_file': {
          if (args.confirm !== true) throw new Error('Set confirm=true to delete a file.');
          if (!args.fileId) throw new Error('fileId required');

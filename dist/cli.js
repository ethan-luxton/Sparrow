...
program
    ...
    let updated = { ...cfg };
    if (answers.openai)
        updated = setSecret(updated, 'openai.apiKey', answers.openai);
    if (answers.telegram)
        updated = setSecret(updated, 'telegram.botToken', answers.telegram);
    updated.user = {
        ...(updated.user ?? {}),
...
configCmd
    ...
    try {
        if (pathKey === 'openai.apiKey' || pathKey === 'telegram.botToken') {
            console.log(getSecret(cfg, pathKey));
            return;
        }
        const value = pathKey.split('.').reduce((acc, k) => acc?.[k], cfg);
        console.log(JSON.stringify(value, null, 2));
    }
    catch (err) {
        logger.error(`cli.config.get failed path=${pathKey} err=${err.message}`);

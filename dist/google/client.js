...
import { getSecret, saveConfig, setSecret } from '../config/config.js';
import { logger } from '../lib/logger.js';
export function buildOAuthClient(cfg) {
    const clientId = cfg.google?.clientId;
    if (!clientId)
        throw new Error('Google clientId missing. Run `sparrow google auth`.');
            ...
            oauth2Client.on('tokens', (newTokens) => {
                if (newTokens.refresh_token || newTokens.access_token) {
                    ...
                    try {
                        ...
                        logger.error(`Failed to save refreshed token: ${err.message}`);
                    }
                }
            });
        }
        catch (err) {
            logger.error(`Failed to load stored Google token: ${err.message}`);
        }
    }
    return oauth2Client;
}

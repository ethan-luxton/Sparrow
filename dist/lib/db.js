...
export function getMessages(chatId, limit = 10) {
    const database = ensureDb();
    const rows = database
...
export function logTool(chatId, tool, action, payload, result) {
    const database = ensureDb();
    database
...
export function listChats() {
    const database = ensureDb();
    const rows = database.prepare('SELECT chatId FROM chats').all();
    return rows.map((r) => r.chatId);
}
export function getLastMessageTimestamp(chatId) {
    const database = ensureDb();
    const row = database
...
export function getLastCheckin(chatId) {
    const database = ensureDb();
    const row = database.prepare('SELECT lastCheckin FROM checkins WHERE chatId = ?').get(chatId);
    return row?.lastCheckin ?? null;
}
export function setLastCheckin(chatId, iso) {
    const database = ensureDb();
...
export function getUserProfile(chatId) {
    const database = ensureDb();
    const row = database.prepare('SELECT content FROM user_profiles WHERE chatId = ?').get(chatId);
    return row?.content ?? null;
}
export function setUserProfile(chatId, content) {
    const database = ensureDb();
    database
        .prepare('INSERT INTO user_profiles (chatId, content, updatedAt) VALUES (?, ?, CURRENT_TIMESTAMP) ON CONFLICT(chatId) DO UPDATE SET content=excluded.content, updatedAt=CURRENT_TIMESTAMP')
        .run(chatId, content);
}

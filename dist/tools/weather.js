...
async function fetchCity(key) {
    ...
    if (!res.ok)
        throw new Error(`Weather fetch failed for ${c.name}: ${res.status}`);
    const json = (await res.json());
    return { meta: c, ...json };
}
function summarizeCity(current, hourly, name) {
    ...
    if (typeof temp === 'number')
        parts.push(`now ${temp.toFixed(0)}°F`);
    if (typeof feels === 'number')
        parts.push(`feels ${feels.toFixed(0)}°F`);

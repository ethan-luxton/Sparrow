...
function assertAllowed(target, allowlist) {
...
export function gmailTool() {
    return {
        name: 'gmail',
        description: 'Search/read Gmail, list threads, inspect/save attachments, and safely compose/send email (send requires confirm=true).',
        permission: 'read',
        schema: {
            type: 'object',
            properties: {
            ...
            switch (args.action) {
                ...
                case 'send_raw': {
                    ...
                    if (missing)
                        return missing;
                    if (args.confirm !== true)
                        return 'Set confirm=true to send raw message.';
                    const res = await gmail.users.messages.send({ userId: 'me', requestBody: { raw: args.raw } });
...
function detectType(mimeType, fileName) {
    ...
    if (mt.includes('pdf') || fn.endsWith('.pdf'))
        return 'pdf';
    if (mt.includes('word') || mt.includes('officedocument.wordprocessingml') || fn.endsWith('.docx'))
        return 'docx';
    return 'unknown';
}

...
async function request(path, method, headers, baseUrl, body) {
...
export function n8nTool() {
    return {
        ...
        handler: async (args) => {
            ...
            if (!baseUrl)
                return 'n8n baseUrl not configured (set n8n.baseUrl or N8N_BASE_URL).';
            // Prefer API key; fall back to basic auth if provided
            let apiKey;
            try {
                apiKey = getSecret(cfg, 'n8n.apiKey');
            }
            catch {
            ...
            let basicPass;
            try {
                basicUser = getSecret(cfg, 'n8n.basicUser');
                basicPass = getSecret(cfg, 'n8n.basicPass');
            }
            catch {
            ...
            switch (args.action) {
                ...
                case 'create_workflow': {
                    ...
                    if (args.confirm !== true)
                        return 'Set confirm=true to create a workflow.';
                    if (args.nodes && !Array.isArray(args.nodes))
                        return 'nodes must be an array of node objects.';
                    const body = {
                        name: args.name,
                        nodes: args.nodes ?? [],
